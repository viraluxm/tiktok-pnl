...
import { planSchedulePage } from '@/lib/schedule/scheduleView';
...
export default async function SchedulePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // NEVER gate on hasActiveRules(): a valid token stands on its own. Resolve every section, then
  // render purely from what exists (planSchedulePage). An employee with no recurring rules can still
  // have a one-time shift assigned to them or a claimable board shift, and must see it.
  ...
  // Nothing pending, assigned, or claimable → the sole empty state. Reworded from the old
  // "No schedule set yet" (which implied a permanent absence) to a transient "nothing pending".
  const plan = planSchedulePage({ myShifts: myShifts.length, board: board.length, pending: pendingClaims.length });
  if (plan.isEmpty) {
    return (
      <Shell>
        {header}
      ...
      {myShifts.map((s) => {
        ...
        return (
          <Card key={s.id}>
            ...
            {within24 && (
              // Its own line below the time so it never splits the time row.
  ...
  // Render ONLY the non-empty sections, in the order planSchedulePage chose (an in-flight OT claim
  // leads; then a non-empty board leads over the schedule). Empty sections are omitted entirely —
  // no placeholder card — and both-empty was handled above.
  const byKey: Record<string, React.ReactNode> = { pending, open: openShifts, yours: yourShifts };
  return (
    <Shell>
      {header}
      {plan.sections.map((k) => byKey[k])}
    </Shell>
  );
}
