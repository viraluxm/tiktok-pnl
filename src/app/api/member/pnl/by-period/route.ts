...
// GET /api/member/pnl/by-period?from=&to= — owner-scoped daily P&L for the member 'pnl' scope.
// Calls pnl_by_period_as (089) with the resolved owner ids, NEVER the caller. Gross-margin only:
// net = revenue × 0.94 − cogs (a 6% platform fee), no labor/ads.
export async function GET(req: Request) {
  ...
  const { data, error } = await admin.rpc('pnl_by_period_as', {
