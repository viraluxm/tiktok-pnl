...
export default function CashflowTab({ data, isLoading, dateFrom, dateTo }: CashflowTabProps) {
  ...
  // Deduplicate bank account for header display
  const bankAccount = payments.find(p => p.bankAccount)?.bankAccount || '';

  const visiblePayments = showAllPayouts ? payments : payments.slice(0, 5);
  const remainingCount = payments.length - 5;

  return (
    <div>
      {/* 1. Hero: Take-Home with period label + expandable waterfall */}
      <div className="bg-tt-card border border-tt-border rounded-[14px] backdrop-blur-xl overflow-hidden mb-8">
        ...
        >
          <div>
            <span className="text-xs text-tt-muted uppercase tracking-wide">Take-Home · {periodLabel}</span>
      ...
      {/* 2. Two status cards: On Hold + Paid Out */}
      <div className="grid grid-cols-2 gap-5 mb-8">
        <div className="bg-tt-card border border-tt-border rounded-[14px] p-6 backdrop-blur-xl">
          ...
          <div className="text-xs text-tt-muted mt-1">{fmtInt(unsettled.totalCount)} unsettled orders · all time</div>
        </div>
        <div className="bg-tt-card border border-tt-border rounded-[14px] p-6 backdrop-blur-xl">
          <div className="flex items-center gap-2 mb-2">
          ...
          <div className="text-xs text-tt-muted mt-1">{paidCount} payouts · recent</div>
        </div>
      </div>

      {/* 3. Recent Payouts — last 5 visible, expandable rows */}
      {payments.length > 0 && (
        <div className="bg-tt-card border border-tt-border rounded-[14px] backdrop-blur-xl overflow-hidden">
          <div className="px-6 py-5 border-b border-tt-border flex items-center justify-between">
            ...
            {bankAccount && (
              <span className="text-xs text-tt-muted">{bankAccount}</span>
            )}
          </div>
                ...
                <div key={`${p.id}-${i}`} className="border-b border-[rgba(255,255,255,0.04)] last:border-b-0">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-sm text-tt-text">{formatPayoutDate(p.paidTime || p.createTime)}</span>
                    </div>
                        ...
                        {stmt ? (
                          ...
                          <>
                            ...
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="text-tt-muted">{formatPayoutDate(p.createTime)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span className="text-tt-text">Paid</span>
                              <span className="text-tt-muted">{formatPayoutDate(p.paidTime)}</span>
                            </div>
                            <div className="border-t border-tt-border my-1" />
                            <div className="flex justify-between text-sm font-bold">
          ...
          {remainingCount > 0 && (
            ...
            >
              {showAllPayouts ? 'Show less' : `Show ${remainingCount} more payouts`}
            </button>
          )}
        </div>
      )}
