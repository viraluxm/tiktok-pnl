-- FIX: lensed_unbind 500s on MULTI-SKU orders.
-- The restock insert used external_ref = p_order_id for every SKU line, but the unique
-- constraint uq_sku_batches_source_ref is (org_id, source, external_ref) — no sku_id. So the
-- 2nd line of a multi-SKU order collided with the 1st inside the same transaction (23505),
-- rolling back the whole unbind. Single-SKU orders were unaffected (one insert).
-- Fix: make external_ref unique per (order, sku): p_order_id || ':' || inventory_sku_id.
-- Idempotency is preserved at the (order, sku) grain; a successful unbind deletes the item so
-- it can't re-run, and a failed unbind rolls back leaving no layer.
...
begin
  ...
  loop
    ...
    if v_line.cost is not null then
      ...
      values
        (v_user, v_org, v_line.inventory_sku_id, v_line.qty, v_line.qty, v_line.cost, v_seq, 'unbind_restock', p_order_id || ':' || v_line.inventory_sku_id::text);
    end if;
