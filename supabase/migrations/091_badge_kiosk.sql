...
-- NO partial UNIQUE(code) WHERE active. The GLOBAL unique(code) above is deliberately kept and is
-- NOT redundant: it bars a revoked code from ever being reissued to a *different* employee, which
-- would let a lost badge punch for the wrong person. Codes are revoked, never deleted, never reused.
-- The global unique btree also serves the `where code = ? and active` resolve lookup, so no extra
-- index is needed.
...
-- NO partial UNIQUE(token) WHERE active — same reasoning as employee_badges.code: the GLOBAL
-- unique(token) above bars a revoked token from ever being reissued, and serves the active lookup.
...
-- ---------------------------------------------------------------------------
-- 4. punch_method — how a punch was made. Lives ONLY on employee_time_entries, the raw entry
--    that IS the source of truth: NOT NULL DEFAULT 'tap' so the EXISTING (untouched) lensed_clock_in
--    auto-labels 'tap' with no RPC change; the badge RPC writes 'badge'.
--    NOT mirrored onto shifts: a NULL there would ambiguously mean manual/scheduled shift, a tap
--    punch, OR a badge-in closed by the admin kiosk. The honest label stays on the entry; read
--    punch method by joining the shift back to its time entry, not off the shift row.
-- ---------------------------------------------------------------------------
...
-- shift. punch_method is NOT written to the shift — it stays on the source time entry (see §4).
