-- 089_member_pnl_read.sql
--
-- Owner-scoped variants of the four P&L RPCs, for the member 'pnl' scope. NOT YET WIRED.
--
-- Each is a VERBATIM copy of the LIVE function body (dumped 2026-08-10 via pg_get_functiondef;
-- the repo's 039/040 copies are not authoritative) with ONLY explicit owner predicates injected
-- and an added p_owner_user_ids uuid[] first parameter. The originals scope purely via RLS and are
-- SECURITY INVOKER, so a member (who owns no data) gets nothing via the session client and
-- service_role would bypass scoping entirely — hence these _as siblings, following the
-- pnl_reorder_by_sku_as (087) precedent. Return shapes are identical to the originals.
--
-- Prefix: 085/086 (scheduling), 087 (reorder), 088 (capture_health) are claimed/applied; 089 next.
--
-- These functions carry gross-margin data (revenue/gmv, cogs, and net = revenue*0.94 - cogs, a 6%
-- PLATFORM FEE only — NO labor/ads/affiliate). SECURITY INVOKER; locked to service_role. The four
-- existing pnl_* functions are NOT modified.


-- ────────────────────────────────────────────────────────────────────────────
-- pnl_by_sku_as — pnl_by_sku live body, owner predicates injected.
-- ────────────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────────────
-- pnl_by_show_as — pnl_by_show live body, owner predicates injected.
-- ────────────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────────────
-- pnl_by_period_as — pnl_by_period live body, owner predicates injected.
-- ────────────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────────────
-- pnl_show_hourly_as — pnl_show_hourly live body, owner predicates injected.
-- ────────────────────────────────────────────────────────────────────────────
...
-- Lock all four to the service role (the member pnl route calls them via service_role).
revoke execute on function public.pnl_by_sku_as(uuid[], date, date, text) from public, anon, authenticated;
grant  execute on function public.pnl_by_sku_as(uuid[], date, date, text) to service_role;
revoke execute on function public.pnl_by_show_as(uuid[], date, date, text) from public, anon, authenticated;
grant  execute on function public.pnl_by_show_as(uuid[], date, date, text) to service_role;
revoke execute on function public.pnl_by_period_as(uuid[], date, date, text) from public, anon, authenticated;
grant  execute on function public.pnl_by_period_as(uuid[], date, date, text) to service_role;
revoke execute on function public.pnl_show_hourly_as(uuid[], uuid, text) from public, anon, authenticated;
grant  execute on function public.pnl_show_hourly_as(uuid[], uuid, text) to service_role;
